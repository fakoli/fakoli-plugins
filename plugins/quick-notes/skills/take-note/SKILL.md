...
2. Append it by piping the text via **stdin** (never as a shell argument — stdin avoids quoting/escaping problems with the apostrophes, quotes, and punctuation common in dictation):
   ...
   Use a single-quoted heredoc delimiter that does not occur as a standalone line in the note. Do not put arbitrary note text in a double-quoted shell string; command substitutions would execute.

   ```bash
   python3 "${PLUGIN_ROOT}/scripts/add-note.py" <<'NOTE'
   the full note text,
...
- **Edit:** `python3 "${PLUGIN_ROOT}/scripts/edit-note.py" <id> "corrected text"` — or pipe new text via stdin.
- **Delete:** `python3 "${PLUGIN_ROOT}/scripts/delete-note.py" <id>`
