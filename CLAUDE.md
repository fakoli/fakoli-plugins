...
2. **Update `README.md`** — add the plugin to the correct category table in the "Plugins" section
3. **Update `.claude-plugin/marketplace.json`** — add the plugin entry with name, version, description, source
4. **Set initial version** to `1.0.0` in `.claude-plugin/plugin.json`
5. Run `./scripts/generate-index.sh` to regenerate `registry/index.json`
6. Run `./scripts/validate.sh plugins/<name>` to validate the plugin
7. Run `./scripts/test-path-resolution.sh plugins/<name>` to deep scan paths and hooks
8. Verify no auto-discovered directories (`skills/`, `commands/`, `agents/`) are declared in manifest
9. Verify `hooks`/`mcpServers` paths use `./` prefix (relative to plugin root) and targets exist
10. If plugin has hooks: verify matchers are specific, no `set -e`, timeouts are set
11. Verify the plugin appears in ALL THREE places: `README.md` table, `marketplace.json` plugins array, and `registry/index.json`

### Keeping Sources in Sync (CRITICAL)
These three files must list exactly the same set of plugins:
- `README.md` — the "Plugins" section tables (grouped by category)
- `.claude-plugin/marketplace.json` — the `plugins` array
- `registry/index.json` — auto-generated by `./scripts/generate-index.sh`

If you add or remove a plugin, update all three. Missing a plugin from `README.md` means users can't discover it. Missing from `marketplace.json` means Claude Code can't install it.
