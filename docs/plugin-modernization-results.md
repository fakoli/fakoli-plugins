...
Validation now parses real YAML, checks native packages and resource links, and reports catalog drift without modifying the checkout. CI runs the combined check entrypoint and uses verified current action references. The first hosted run exposed a missing `setup-uv@v10` alias; workflows now use its published `v10.0.1` tag. Workflow assets match the root workflows. The basic template honestly describes an unfinished workflow and validates its actual skill packaging instead of invoking nonexistent Python code/tests.
